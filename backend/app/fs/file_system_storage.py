...
class ArtifactsFileSystemStorage:
    ...
    Usually it doesn't save artifacts by itself but provides a path where it should be saved to.
    ...
    def __create_if_not_exist(self, field_id, data_type):
        try:
            pathlib.Path(os.path.join(self.base_path, str(field_id), data_type)).mkdir(parents=True, exist_ok=True)
    ...
    def get_path_to_satellite_data_unzipped(self, field_id):
        self.__create_if_not_exist(field_id=field_id, data_type=UNZIPPED_FOLDER)
        return os.path.join(self.base_path, str(field_id), UNZIPPED_FOLDER)

    def get_path_to_ndvi_image(self, field_id):
        self.__create_if_not_exist(field_id=field_id, data_type=NDVI_IMAGE_DATA_FOLDER)
        return os.path.join(self.base_path, str(field_id), NDVI_IMAGE_DATA_FOLDER)
